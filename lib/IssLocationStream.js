...
class IssLocationStream extends stream.Readable {
  ...
  handleError(obj) {
    if (obj.errno || obj.status) {
      ...
      ret.error = obj.errno || obj.status;
      ret.message = obj.code || obj.description;
...
var merps = new IssLocationStream('25544', 1000, {calculateChange: true});

merps.on('data', function (theThing) {
  console.log(theThing);
});

merps.on('error', function (e) {
  console.log('blerp error');
  console.log('' + e);
});

merps.on('end', function () {
