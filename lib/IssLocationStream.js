...
class IssLocationStream extends stream.Readable {
  ...
  getDataFromSource() {
    ...
    return this.dataSource.makeRequest(this.satellite)
      .then(resp=> {
        this.resps += 1;
      ...
      .catch(err=> {
        this.push('ERR0R: ' + err + ' ]X[ ' + err.message);
      })
...
var merps = new IssLocationStream('', 100);

merps.on('data', function (theThing) {
  console.log('requests: ' + merps.reqs);
  console.log('responses: ' + merps.resps);  
